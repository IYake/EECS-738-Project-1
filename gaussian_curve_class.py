...
def plot_curves(figureNum,df,X_label,Y_label,class_feature,class1,class2,curve1,curve2):
    ...
    #plot points
    bx = class1_points.plot(kind='scatter',x = X_label, y = Y_label,color='red', label = class1)
    class2_points.plot(kind='scatter',x = X_label, y = Y_label, color='blue', ax=bx, label=class2)
    plt.plot(curve1.mu[0],curve1.mu[1],'co',markersize=6, label = 'cluster mean')
    plt.plot(curve2.mu[0],curve2.mu[1],'co',markersize=6)
    plt.legend(loc='best')
