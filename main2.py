...
df = pd.read_csv('data/winequality-red.csv', usecols = ['quality', Y_label, X_label])

good = df.loc[df['quality'] >= 7 ]
bad = df.loc[df['quality'] <= 4 ]
frames = [good, bad]
wine = pd.concat(frames, ignore_index = True)

#create random means and sigmas and gaussian weighting factors
numClasses = 2
xMax, xMin, yMax, yMin = wine.loc[:,X_label].max(), wine.loc[:,X_label].min(), wine.loc[:,Y_label].max(), wine.loc[:,Y_label].min()
...
numPoints = wine.shape[0]
if (numPoints % 2 != 0):
    X = wine.loc[:numPoints-2,X_label].values
    Y = wine.loc[:numPoints-2,Y_label].values
else:
    X = wine.loc[:numPoints,X_label].values
    Y = wine.loc[:numPoints,Y_label].values
